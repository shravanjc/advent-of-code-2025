...
input.replace(System.lineSeparator(), "")
    ...
    .forEach { range ->
        ...
        val from = rangeValues[0].toLong()
        val to = rangeValues[1].toLong() + 1
        for (count in from until to) {
            val countString = count.toString()
            val firstHalf = countString.substring(0, countString.length / 2)
            val secondHalf = countString.substring(countString.length / 2, countString.length)
            if (firstHalf == secondHalf) {
                println(countString)
                answer += count
                continue
            } else {
                val splitLoop = countString.length / 2
                for (splitCounter in 1 until splitLoop + 1) {
                    val duplicate = countString.chunked(splitCounter).toSet().size == 1
                    if(duplicate) {
                        println(countString)
                        answer += count
                        continue
